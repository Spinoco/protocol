...
object ResentFromSpec extends Properties("ResentFrom") {
  ...
  implicit val HeaderCodec = ResentFrom.codec

  property("single-email") = protect {

    verify(
      "John Doe <john.doe@spinoco.com>"
      , ResentFrom(EmailAddress("john.doe", "spinoco.com", Some("John Doe")), Nil)
    )
  ...
  property("multiple-email") = protect {
    verify(
      ...
      , ResentFrom(EmailAddress("john.doe", "spinoco.com", Some("John Doe")), List(EmailAddress("jannet.doe", "spinoco.com", None)))
