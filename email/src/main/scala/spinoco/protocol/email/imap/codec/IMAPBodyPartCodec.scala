package spinoco.protocol.email.imap.codec
...
import spinoco.protocol.email.imap.BodyStructure._
...
object IMAPBodyPartCodec {
  ...
  lazy val bodyStructure: Codec[BodyPart] = {
    `(` ~> constantStringCaseInsensitive("BODYSTRUCTURE") ~> SP ~> codec <~ `)`
  }
  ...
  object impl {
    ...
    val `(` : Codec[Unit] = "(" | constantString("(")
    val `)` : Codec[Unit] = ")" | constantString(")")
    val `{` : Codec[Unit] = "{" | constantString("{")
    val `}` : Codec[Unit] = "}" | constantString("}")
    val `}crlf` : Codec[Unit] = "}crlf" | constantString("}\r\n")
    val SP  : Codec[Unit] = "SP" | constantString(" ")
    val NIL : Codec[Unit] = "NIL" | constantStringCaseInsensitive("NIL")
    ...
    val DQUOTE  : Codec[Unit] = "DQUOTE" | constantString("\"")
    ...
    def  bodyFldParam: Codec[Vector[(String, String)]] = {
      choice(
        nilVector
        , `(` ~> vectorVDelimited(bits_SP, string ~ (SP ~> string)) <~ `)`
      )
    }
    ...
    def bodyFldLang:  Codec[Vector[String]] = {
      choice(
        `(` ~> minItems(2)(vectorVDelimited(bits_SP, string)) <~ `)`
        ...
        bodyFldMD5 ~
          optional(lookahead2(SP), SP ~> bodyFldDsp ~
            optional(lookahead2(SP), SP ~> bodyFldLang ~
              optional(lookahead2(SP), SP ~> bodyFldLoc ~ vectorVDelimited(bits_SP, bodyExtension) )
            )
        )
      ).xmap(
        { case (md5, next) =>
    ...
    def mediaText: Codec[String] = {
      DQUOTE ~> constantStringCaseInsensitive("TEXT") ~> DQUOTE ~> SP ~> string
    }

    def mediaMessage: Codec[Unit] = {
      DQUOTE ~> constantStringCaseInsensitive("MESSAGE") ~> DQUOTE ~> SP ~>
      DQUOTE ~> constantStringCaseInsensitive("RFC822") ~> DQUOTE
    }
