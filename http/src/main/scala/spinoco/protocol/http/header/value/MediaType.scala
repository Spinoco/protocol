...
object MediaType {
  ...
  val mediaTypes:Map[String,Map[String,MediaType]] = Seq(
  ...
  val codec: Codec[MediaType]  = {
    ...
    val slash = ByteVector('/').bits

    def encode(in:MediaType):Attempt[List[BitVector]] = {
      def toBits(vs:String*):Attempt[List[BitVector]] = {
        Attempt.successful(vs.toList.map { s => BitVector.view(s.getBytes) })
      }
      in match {
        case DefaultMediaType(main, sub, _, _, _) => toBits(main,sub)
        case MultipartMediaType(sub, _) => toBits("multipart", sub)
        case CustomMediaType(main, sub) =>  toBits(main, sub)
      }
    }

    def decode(bv:List[BitVector]):Attempt[MediaType] = {
      if (bv.size != 2) Attempt.failure(Err(s"Expected <mime>/<sub>, got $bv"))
      else Attempt.fromEither {
        bv.head.decodeAscii.right.flatMap { main =>
        bv.last.decodeAscii.right.map { sub  =>
          mediaTypes.get(main).flatMap { mainMt =>
            mainMt.get(sub)
          }.getOrElse(CustomMediaType(main, sub))

        }}.left.map(x => Err(x.toString))
      }
    }

    listDelimited(slash, bitsWsRemoved).exmap(decode,encode)
  }
